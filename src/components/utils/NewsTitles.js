...
function NewsTitles (props) {
  ...
  if (RawData.length === 0) {
    ...
    function ProcessRelationship (RelType) {
      if (RelType.includes('opposes')) { return Opposes } else if (RelType.includes('supports')) { return Supports }
      return Neutral
    }

    function ProcessArticleLink (RelType) {
      if (RelType.startsWith('https://publico.pt')) { return <img width="15" src={PublicoLogo} alt="publico.pt"/> }
      return <img width="35" src={ArquivoLogo} alt="arquivo.pt"/>
    }

    const headlines = RawData.map(RawData => (
      {
